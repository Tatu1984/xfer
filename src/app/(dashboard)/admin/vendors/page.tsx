...
import { Search, MoreHorizontal, Store, Eye, Ban, DollarSign, Loader2, CheckCircle, Building2 } from "lucide-react";
...
export default function AdminVendorsPage() {
  ...
  const handleStatusChange = async (vendorId: string, newStatus: string) => {
    try {
      ...
      if (response.ok) {
        toast.success(`Vendor ${newStatus === "SUSPENDED" ? "suspended" : "reactivated"}`);
        fetchVendors();
      } else {
        const data = await response.json();
